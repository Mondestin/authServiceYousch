...
def check_mysql_connection():
    ...
    if not result:
        print("❌ MySQL is not accessible. Please ensure MySQL is running.")
        return False
...
def update_env_file():
    ...
    if "DATABASE_URL=" in content:
        ...
        print("✅ Updated .env file with local MySQL settings")
        return True
    else:
...
def main():
    ...
    if not check_mysql_connection():
        print("\n❌ Setup failed. Please ensure MySQL is running and accessible.")
        sys.exit(1)
    
    # Create database and user
    if not create_database_and_user():
        print("\n❌ Setup failed. Could not create database or user.")
        sys.exit(1)
